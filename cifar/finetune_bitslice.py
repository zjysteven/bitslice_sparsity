...
def main():
    ...
    for epoch in range(args.start_epoch, args.epoches):
        ...
        if is_best:
            ...
            save_checkpoint(
                ...
                filename=os.path.join(
                    args.save_dir,
                    "finetune_bitslice_{}_{}.tar".format(args.arch, args.th),
                ),
            )
        ...
        #np.save("bitslice_curve_{}_{}.npy".format(args.arch, args.th), train_curve)
...
def train(train_loader, model, criterion, optimizer, epoch, masks):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            print(
                "\rEpoch: [{0}]\tLoss: {loss.avg:.4f}\tL1: {l1:.4f}\tPrec: {top1.avg:.2f}%\tZero_ratio: {a:.2f}%, {b:.2f}%, {c:.2f}%, {d:.2f}%".format(
                    epoch,
                    loss=losses,
                    l1=l1_reg.item()/np.sum(total_param_cnt),
                    top1=top1,
                    a=z1,
                    b=z2,
                    c=z3,
